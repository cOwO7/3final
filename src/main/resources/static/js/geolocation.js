			...
			navigator.geolocation.getCurrentPosition(
				(position) => {
					...
					fetch('/getWeather', {
						...
						.then((response) => {
							if (!response.ok) { // 응답이 OK가 아닌 경우
								return response.json().then((data) => { // 응답을 JSON으로 파싱
									console.error("서버 오류:", data.message || "알 수 없는 오류");
									throw new Error('날씨 데이터를 가져오지 못했습니다: ' + (data.message || response.statusText)); // 오류 처리
								});
							}
						...
						.catch((error) => {
							console.error("에러:", error); // catch에서 모든 오류 처리
						});
				},
				(error) => {
